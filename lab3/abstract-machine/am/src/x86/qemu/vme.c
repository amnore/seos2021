...
void *krnl_addrof(AddrSpace *as, uintptr_t addr) {
  uintptr_t *pt = (uintptr_t *)as->ptr;
  for (int i = 1; i <= mmu.ptlevels; i++) {
    const struct ptinfo *info = &mmu.pgtables[i];
    int index = indexof(addr, info);

    if (i == mmu.ptlevels) {
      return (void *)(baseof(pt[index]) + addr - (addr & (mmu.pgsize - 1)));
    }

    if (!(pt[index] & PTE_P)) {
      return NULL;
    }
    pt = (uintptr_t *)baseof(pt[index]);
  }
...
// recursively copy page table
static uintptr_t *_copyvm(uintptr_t *pgtbl, int level) {
  uintptr_t *page = pgallocz();
  panic_on(!page, "failed to allocate page");

  for (int i = 0; i < mmu.pgsize / sizeof(uintptr_t); i++) {
    page[i] = pgtbl[i];
    if ((page[i] | PTE_P) && level != mmu.ptlevels) {
      assert(_copyvm((uintptr_t *)baseof(page[i]), level + 1));
    }
  }
...
void copyvm(AddrSpace *dst, AddrSpace *src) {
  panic_on(!dst || !src, "NULL AddrSpace");

  *dst = (AddrSpace){
      .pgsize = src->pgsize, .area = src->area, .ptr = _copyvm(src->ptr, 1)};
}

void ucontext_reset(Context *ctx, AddrSpace *as, Area kstack, void *entry,
                    void *ustack_top) {
  *ctx = (Context){0};
...
Context *ucontext(AddrSpace *as, Area kstack, void *entry) {
  ...
  ucontext_reset(ctx, as, kstack, entry, uvm_area.end);
  return ctx;
}
